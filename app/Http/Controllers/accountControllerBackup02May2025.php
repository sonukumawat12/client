...
class AccountController extends Controller
{
    ...
    public function show($id, Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            // This part will work if realize cheque issue work
            ...
            // This part will work if transaction_id has no data and 
            $accounts = $accounts->where(function ($query) use ($realize_cheque) {
                $query->whereNull('account_transactions.transaction_payment_id')
                ->orWhere(function ($query2) use ($realize_cheque) {
                    $query2->whereNotNull('account_transactions.transaction_payment_id');
            ...
            if (!empty($start_date) && !empty($end_date)) {
                if (request()->date_based_on == 'transaction_date') {
                    ...
                    $accounts->whereNull('TP.update_post_dated_cheque');
                } else {
            ...
            $modifiedAccounts = $accounts->flatMap(function ($row, $index) use ($business_id, $currency_precision,$id,$start_date,$end_date,$today) {  
                        ...
                        if ($settlement) {
                            ...
                            foreach ($transactions as $transaction) {
                                ...
                                $newRows = array_merge($rowData, [
                                    'id' => $id ?? null, // Ensure id is retained
                ...
                // dd(collect([$row]));
                // dd(collect([$row])->merge($newRow));
                // return collect([$row])->merge($newRow);
            ...
            return DataTables::of($modifiedAccounts)
                ...
                ->addColumn('credit', function ($row) use ($business_details, $currency_precision,$id,$start_date,$end_date) {
                    if($row->post_dated_cheque== 1){
                        $debit = $row->amount;
                        // Session::put('daily_collection', $total_daily_collection);
                        return  '<span class="display_currency credit_col" data-currency_symbol=false data-orig-value="' . $debit . '" >' . $debit . '</span>';
                    }else{
                ...
                ->addColumn('cheque_number', function ($row) {
                  $slipNo = null;
                    ...
                    if (!empty($row->transaction)) {
                        $slipNo = AccountTransaction::where('transaction_id', $row->transaction->id)
                            ->whereNotNull('slip_no')
                            ->orderBy('id') // optional: get the earliest
                            ->value('slip_no');
                    }
                    $transaction_payment = $row->transaction 
                        ? TransactionPayment::where('transaction_id', $row->transaction->id)->first() 
                        : null;
                    $transaction_payment_method = TransactionPayment::where('transaction_id', $row->transaction->id)->first(); 
                    ...
                    if ($row->sub_type === 'deposit') {
                        ...
                        if (!empty($tp)) {
                            if ($tp->method === 'card') {
                                return $tp->card_transaction_number ?? '';
                            }
                ...
                ->addColumn('description', function ($row) use ($business_id, $id, $currency_precision,$start_date,$end_date) {
                    if($row->transaction_id == null && $row->employee_advance_id != null):
                        $chkAdvance = EssentialsEmployeeAdvance::where(['account_id'=>$id])
                        ->whereBetween(DB::raw('date(essentials_employee_advances.datetime_entered)'), [$start_date, $end_date])
                        ->get();
                        if($chkAdvance):
                            // return $accounts->get();
                            $accounts = $accounts->join('essentials_employee_advances','account_transactions.account_id','essentials_employee_advances.account_id')
                            ...
                            ->where('essentials_employee_advances.account_id',$id)
                            ->select([
                    ...
                    if($chkAdvance):
                        ...
                        if ($row->type == 'debit' && $row->txnType == 'advance') {
                            $details .= '<b>Payment Type:</b> ' . $row->EPS_name . '<br>';
                            $details .= '<b>Payment Method: </b> ' .$row->account_name. '<br>';
                            $details .= '<b>Salary Period:</b> ' . $row->salary_period_start .' to ' .$row->salary_period_end ;
                        }

                        if ($row->type == 'credit' && $row->txnType == 'advance') {
                            $details .= '<b>Payment Type:</b> ' . $row->EPS_name . '<br>';
                            $details .= '<b>Salary Period:</b> ' . $row->salary_period_start .' to ' .$row->salary_period_end ;
                            $details .= '<b>Liability or Expense Account: </b> ' .$row->account_name. '<br>';
                        }
                    endif;
                            ...
                            if (in_array($row->sub_type, ['fund_transfer', 'cheque_realize', 'deposit', 'payable', 'stock', 'opening_balance']) && ((!empty($row->transaction) && $row->transaction->is_settlement != 1) || empty($row->transaction))) {
                                ...
                                if (in_array($row->sub_type, ['deposit']) && !empty($row->transfer_transaction)) {
                                    ...
                                    if ($row->account_id == $cash_account_id && !empty($row->cheque_number)) {
                                        ...
                                        $details .= '<br><b>' . __('cheque.cheque_number') . ': </b>' . $cheque_number;
                                    }
                                }
                                ...
                                if (!empty($row->transaction->type)) {
                                    ...
                                    elseif ($row->transaction->type == 'purchase_return') {
                                        ...
                                        $details .= '<b>' . __('contact.customer') . ':</b> ' . $contact->name;
                                    }elseif ( $row->transaction->type == 'hms_booking'){
                ...
                ->removeColumn('id')
                ->removeColumn('is_closed')
                ->rawColumns(['opening_balance', 'id', 'note', 'credit', 'debit', 'balance', 'sub_type','action', 'attachment', 'reconcile_status', 'description'])
                ->make(true);
            }
        ...
        if (isset($account) && $account->is_main_account == 0) {
            return view('account.show')
                ->with(compact('is_iframe', 'slipNos', 'account_access', 'account', 'card_account_id', 'card_type_accounts', 'id', 'ac_id', 'cheque_in_hand_group_id', 'bank_group_id', 'cheque_return_account_id', 'cheque_numbers', 'customers', 'suppliers'));
        } else {
                ...
                if (!empty($select_cheque)) {
                    ...
                    if (!empty($transaction_payment)) {
                        ...
                        if ($account_transaction->post_dated_cheque == 1) {
                            ...
                            $from_account = Account::getAccountByAccountName('Post Dated Cheques')->id;
                            // $this->transactionUtil->account_exist_return_id('Post Dated Cheques');
                        }else{
            ...
            foreach ($select_cheques as $select_cheque) {
                if (!empty($select_cheque)) {
                    ...
                    if (!empty($account_transaction)) {
                        ...
                        //work for get realize cheque bank id as account id column by virtual it professional referance docs number 7338
                        $checkRealiseBank = $request->input("realize_cheque_bank_$select_cheque");
                        if(empty($checkRealiseBank)):
                            $checkRealiseBank = 0;
                        endif;
                        // working on this part as a virtual it professional docs number 7338
                        $credit_data = [
                            'amount' => $account_transaction->amount,
                            'account_id' => $checkRealiseBank,
                            'type' => 'credit',
    ...
    public function getBankAccountByGroupDP()
    {
        ...
        if (!empty($group_name)) {
            if ($group_name == 'credit_sale') {
                ...
                // dd("the groupid is: ".$group_id);
                if (!empty($group_id)) {
                    $accounts = Account::getAccountByAccountGroupId($group_id);
                    // dd("the account is: ".json_encode($accounts));
                } else {
                    $accounts = [];
                }
            }
    ...
    public function getBankAccountByGroupDPNew()
    {
        $group_name = request()->group_name;
        $location_id = request()->location_id;
        $business_id = session()->get('user.business_id');
        ...
        if (!empty($group_name)) {
            $accounts = Account::where('name', $group_name)
                ->where('business_id', $business_id)
                ->select('id', 'name')
                ->pluck('name', 'id');
        }
