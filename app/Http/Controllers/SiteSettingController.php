...
class SiteSettingController extends Controller
{
    public function viewPage()
    {
        ...
        if (tenant()) {
            ...
            $settings = SiteSettings::on('mysql')->where('id', 1)->first(); // replace 'mysql' with your main connection name
        }
        // $settings = SiteSettings::where('id', 1)->first();
    ...
    public function updateSettings(Request $request)
    {
        ...
        try {
            ...
            if (tenant()) {
                // Tenant context: Already using tenant DB
                $settings = SiteSettings::updateOrCreate(['id' => 1], $filter_data);
            } else {
                // Super Admin / base system: use central connection
                $settings = SiteSettings::on('mysql')->updateOrCreate(['id' => 1], $filter_data);
            }
