...
class TempController extends Controller
{
    ...
    public function saveSettlementSWTemp(Request $request){
        $business_id = request()->session()->get('user.business_id');
        TempData::updateOrCreate(['business_id' => $business_id],['settlement_sw_data' => json_encode($request->except('_token'))]);
        return json_encode(['status' => true]);
    }
    ...
    public function saveStockTransferTemp(Request $request){
        $business_id = request()->session()->get('user.business_id');
        $result=TempData::updateOrCreate(['business_id' => $business_id],['stock_transfer_data' => json_encode($request->except('_token'))]);
       \Log::info("temp data{ $result}");
        return json_encode(['status' => true]);
    }
