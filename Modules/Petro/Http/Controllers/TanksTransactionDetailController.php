...

{
...

    {
        ...
        if (request()->ajax()) {
            ...
            $tanks_transaction_details = Datatables::of($sell_query)
                ...
                ->editColumn('purchase_qty', function ($row) use ($business_details) {
                    if (
                        ...
                        ($row->purchase_qty == 0 && $row->type == 'stock_adjustment')
                    ) {
                        $testing_qty = 0;
                        ...
                        if ($testing_qty > 0) {
                            $formatted = $this->productUtil->num_f($testing_qty, false, $business_details, true);
                            return "<span class='purchase_qty_transaction' data-orig-value='{$testing_qty}'>{$formatted}</span> <span class='label bg-light-green' style='margin-left: 5px'>Testing</span>";
                        } else {
                            return "<span class='purchase_qty_transaction' data-orig-value='0'>0</span>";
                        }
                
                    } else {
                        $raw_value = $row->purchase_qty;
                ...
                ->editColumn('sold_qty', function ($row) use ($business_details) {
                    if (
                        ...
                        ($row->sold_qty == 0 && $row->type == 'stock_adjustment')
                    ) {
                        return "<span class='sold_qty_transaction' data-orig-value='0'>0</span>";
                    } else {
                        $raw_value = abs($row->sold_qty);
                        $formatted = $this->productUtil->num_f($raw_value, false, $business_details, true);
            ...
            return $tanks_transaction_details->rawColumns(['transaction_date', 'balance_qty', 'ref_no','purchase_order_no','sold_qty', 'purchase_qty'])
        ...
        if (request()->ajax()) {
            ...
            if (request()->ajax()) {
                ...
                $tanks_transaction_details = Datatables::of($tanks)
                    ...
                    ->editColumn('purchase_qty', function($row) use($business_details){
                        $raw_value = $row->purchase_qty + $row->testing_qty;
                        $value = number_format($raw_value, 3, '.', ',');
